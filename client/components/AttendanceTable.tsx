...
export default function AttendanceTable() {
  ...
  const parsePastedData = (text: string) => {
    try {
      ...
      if (parsed.length === 0) {
        setError(
          "No valid data found. Make sure to paste tab-separated values.",
        );
        return;
      }
  ...
  const calculateMetrics = () => {
    ...
    const validRows = tableData.filter(
      (row) => row && row.name && row.regNo && row.date && row.time,
    );
    ...
    Object.entries(grouped).forEach(([key, groupRows]) => {
      ...
      for (let i = 0; i < sortedTimes.length - 1; i += 2) {
        ...
        if (i + 2 < sortedTimes.length) {
          ...
          const breakMinutes =
            (nextInTime.getTime() - outTime.getTime()) / (1000 * 60);
          if (breakMinutes > 0) {
    ...
    <div className="w-full max-w-screen-2xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="space-y-2">
        ...
        <p className="text-lg text-slate-600">
          Click any cell to edit like Excel. Add/remove rows as needed.
        ...
        <CardHeader className="bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200">
          <div className="flex justify-between items-center">
            <CardTitle className="text-lg">
              Attendance Data (Click cells to edit)
            </CardTitle>
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-gradient-to-r from-slate-100 to-slate-50 border-b-2 border-slate-300">
                  <th className="px-4 py-3 text-left text-sm font-bold text-slate-700 w-16 border-r border-slate-200">
                    S No
                  </th>
                  <th className="px-4 py-3 text-left text-sm font-bold text-slate-700 border-r border-slate-200">
                    Name
                  </th>
                  <th className="px-4 py-3 text-left text-sm font-bold text-slate-700 w-24 border-r border-slate-200">
                    Reg No
                  </th>
                  <th className="px-4 py-3 text-left text-sm font-bold text-slate-700 w-32 border-r border-slate-200">
                    Date
                  </th>
                  <th className="px-4 py-3 text-left text-sm font-bold text-slate-700 w-32 border-r border-slate-200">
                    Time
                  </th>
                  <th className="px-4 py-3 text-center text-sm font-bold text-slate-700 w-32">
                    Action
                  </th>
                </tr>
              </thead>
                ...
                {tableData.map((row, idx) => (
                  ...
                  >
                    <td className="px-4 py-3 text-sm text-slate-700 font-medium border-r border-slate-200 w-16">
                      <div className="bg-slate-100 rounded px-2 py-1 text-center">
                        {row.sNo}
                      </div>
                    </td>
